...
- Added `font list --discovery auto|native|fontconfig`.
- Added `font list --debug` with selected discovery mode, adapter attempts, durations, and sanitized failure details.
- Added concise default `auto` info text when macOS uses fontconfig or falls back to native discovery.
...
- Passed `bun test test/fonts.test.ts` with 22 tests.
- Passed `bun test test/cli-actions-md-to-pdf.test.ts` with 53 tests.
- Passed `bun test test/fonts.test.ts test/cli-actions-md-to-pdf.test.ts` with 75 tests.
- Passed `bun run lint`.
...
- `--discovery`, `--debug`, debug JSON fallback attempts, default `auto` info text, sanitized failed attempts, and explicit discovery-mode behavior are covered in [fonts.test.ts](../../../test/fonts.test.ts).
...
## Phase 7 Follow-Up Completion

The initial `font list` command now exposes discovered font candidates and the Phase 7 follow-up diagnostics are implemented. The design rationale is captured in [Font Command Discovery Options](../../researches/research-2026-05-07-font-command-discovery-options.md).

Completed follow-up decisions and behavior:
