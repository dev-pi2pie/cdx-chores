...
As of `v0.1.4`, this guide documents the stable split where direct `data query` owns SQL execution, accepted header-mapping reuse, and accepted source-shape replay, while reviewed source-shape generation still begins on the `data extract` lane.
