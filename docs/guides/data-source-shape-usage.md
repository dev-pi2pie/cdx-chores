...
The current shipped direct-CLI split is: `data extract` produces reviewed source-shape artifacts, `data extract` and `data query` can both replay them, and `data query codex` stays on explicit shape flags instead of artifact replay.
