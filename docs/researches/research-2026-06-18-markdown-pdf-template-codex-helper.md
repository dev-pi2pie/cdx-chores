...
- the command should accept the same deterministic recipe flags as `md pdf-template init` and `md to-pdf` for page shape, preset, and ToC settings. ToC settings are later-render recipe settings, not static ToC content generated by template-Codex.
...
- the custom template bundle is the command output. The effective output directory comes from `--output <directory>` or from a generated default, and the v1 helper should generate that bundle from deterministic template families, recipe flags, profile signals, document signals, and managed cover assets.
