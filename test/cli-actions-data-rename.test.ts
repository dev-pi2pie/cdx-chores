...
describe("cli action modules: rename", () => {
  ...
  test("actionRenameFile codex auto routes markdown to the document analyzer", async () => {
    const fixtureDir = await createTempFixtureDir("actions");
    let planCsvPath: string | undefined;
    try {
      const { runtime, stdout, stderr } = createCapturedRuntime();
      const dirPath = join(fixtureDir, "rename-file-codex-auto-doc");
      await mkdir(dirPath, { recursive: true });
  ...
  test("actionRenameBatch codex auto routes image-only input to the image analyzer", async () => {
    const fixtureDir = await createTempFixtureDir("actions");
    let planCsvPath: string | undefined;
    try {
      const { runtime, stdout, stderr } = createCapturedRuntime();
      const dirPath = join(fixtureDir, "rename-codex-auto-images");
      await mkdir(dirPath, { recursive: true });
  ...
  test("actionRenameBatch codex auto routes docs-only input to the document analyzer", async () => {
    const fixtureDir = await createTempFixtureDir("actions");
    let planCsvPath: string | undefined;
    try {
      const { runtime, stdout, stderr } = createCapturedRuntime();
      const dirPath = join(fixtureDir, "rename-codex-auto-docs");
      await mkdir(dirPath, { recursive: true });
