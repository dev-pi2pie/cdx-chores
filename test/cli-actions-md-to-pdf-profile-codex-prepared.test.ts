...
describe("Markdown PDF prepared Profile Codex services", () => {
  ...
  test("resolves a late Profile collision without changing its usable extension", async () => {
    await withTempFixtureDir("md-pdf-profile-codex-prepared-late-output", async (fixtureDir) => {
      await writeFile(join(fixtureDir, "report.md"), "# Report\n", "utf8");
      const { runtime } = createActionTestRuntime({
        cwd: fixtureDir,
      ...
      const prepared = await prepareMarkdownPdfProfileCodex(runtime, {
        ...
        input: "report.md",
        intent: "wide table report",
        profileUidFactory: () => "md-pdf-profile-20260722T090000Z-late",
      });
      expect(prepared.kind).toBe("profile");
      if (prepared.kind !== "profile") {
        throw new Error("Expected a prepared profile.");
      }
      await writeFile(prepared.suggestedOutputPath, "occupied\n", "utf8");

      const output = await suggestedMarkdownPdfCodexOutputPath({
        artifact: "profile",
        prepared,
        setup: { artifact: "profile", fontHints: [] },
      });
      const destination = await bindMarkdownPdfProfileCodexDestination(runtime, prepared, {
        output,
        report: { kind: "none" },
      ...
      expect(output).toBe(prepared.suggestedOutputPath.replace(/\.yml$/, "-1.yml"));
      expect(destination.outputPath).toBe(output);
      expect(runnerCalls).toBe(1);
      expect(await readdir(fixtureDir)).toEqual([
        "report.md",
        "report-md-pdf-profile-20260722T090000Z-late.yml",
      ]);
    });
  });
});
