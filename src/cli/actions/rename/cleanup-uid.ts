...
const CANONICAL_CLEANUP_UID_BASENAME_PATTERN = /^uid-([0-9a-hjkmnpqrstvwxyz]{10,16})$/i;
...
export async function buildCleanupUidBasenames(sourcePath: string): Promise<string[]> {
  ...
  const canonicalMatch = CANONICAL_CLEANUP_UID_BASENAME_PATTERN.exec(sourceStem);
  if (canonicalMatch?.[1]) {
