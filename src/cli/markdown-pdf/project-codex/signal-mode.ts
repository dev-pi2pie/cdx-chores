...
import type { MarkdownPdfProjectCodexTemplateOwnedSignals } from "./types-signals";

const TEMPLATE_OWNED_INTENT_PATTERNS: Array<{ label: string; pattern: RegExp }> = [
...
export function collectTemplateOwnedProjectDirections(input: {
  ...
  const intentDirections = TEMPLATE_OWNED_INTENT_PATTERNS.filter(({ pattern }) =>
    pattern.test(intent),
  ).map(({ label }) => label);
