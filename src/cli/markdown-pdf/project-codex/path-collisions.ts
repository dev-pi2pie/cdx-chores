...
export async function assertUsableProjectCodexOutputDirectory(
  ...
  try {
    ...
    if (stats.isSymbolicLink()) {
      throw new CliError(`Project output directory is a symlink: ${outputDirectory}`, {
        code: "OUTPUT_SYMLINK",
        exitCode: 2,
      });
    }
    if (!stats.isDirectory()) {
      throw new CliError(`Project output path is not a directory: ${outputDirectory}`, {
        code: "INVALID_INPUT",
        exitCode: 2,
      });
    }
    if (!options.overwrite) {
      ...
      if (entries.length > 0) {
        throw new CliError(
          `Project output directory is not empty: ${outputDirectory}. Use --overwrite to replace project files.`,
          {
            code: "OUTPUT_EXISTS",
            exitCode: 2,
          },
    ...
    throw new CliError(
      `Failed to inspect project output directory: ${outputDirectory} (${message})`,
      {
