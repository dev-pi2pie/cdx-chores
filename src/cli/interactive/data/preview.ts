...
import { actionDataParquetPreview, actionDataPreview, loadDataPreviewSource } from "../../actions";
import {
...
async function promptWindowInputs(): Promise<{
  ...
  return {
    ...
    offset: parseOptionalPositiveInteger(offsetInput),
    rows: parseOptionalPositiveInteger(rowsInput),
  };
}

export async function runInteractiveDataPreview(
  runtime: CliRuntime,
...
export async function runInteractiveParquetPreview(
  runtime: CliRuntime,
  pathPromptContext: InteractivePathPromptContext,
): Promise<void> {
