...
export function buildMarkdownPdfTemplateCodexPrompt(
  ...
  const facts = {
    ...
    coverCompositionPolicy: {
      ...
      imageAnchor: MARKDOWN_PDF_TEMPLATE_CODEX_COVER_IMAGE_ANCHORS,
      mediaScale: MARKDOWN_PDF_TEMPLATE_CODEX_COVER_MEDIA_SCALES,
      rules: [
        "Use cover.composition to express cover HTML order; do not invent raw layout names.",
    ...
    fontDecisionPolicy: {
      ...
      languageRule:
        "pdf.content-langs is expected coverage, not precise text labeling; language-keyed body fonts only target rendered lang attributes such as Pandoc spans.",
      rule: "Return font_decisions only for bounded body, heading, or code family choices using the role/key matrix. Loose font hints must not override concrete base-profile fonts for the same role/key unless template_level is true and the choice is explicitly template-owned.",
    },
    ...
    slotEnums: {
      ...
      coverMediaAligns: MARKDOWN_PDF_TEMPLATE_CODEX_COVER_MEDIA_ALIGNS,
      coverMediaScales: MARKDOWN_PDF_TEMPLATE_CODEX_COVER_MEDIA_SCALES,
      coverTextAligns: MARKDOWN_PDF_TEMPLATE_CODEX_COVER_TEXT_ALIGNS,
  ...
  return [
    ...
    "- font_decisions must use role/key pairs from fontDecisionPolicy.roleKeyMatrix: body.default, body.<valid-language-tag>, code.default, code.symbols, and heading.default.",
    "- font_decisions may name one family per body/key, heading/default, or code/key pair; do not return raw font-family CSS stacks.",
    "- Set font_decisions[].template_level true only with source template-style, when the template should explicitly own that font role/key over a base-profile font.",
